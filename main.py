...
░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░░╚═════╝░░░░╚═╝░░░░╚════╝░░╚════╝░╚══════╝╚═╝░░╚═╝░╚═════╝░╚═════╝░
    ...
    print_border("AFYNITI SOCIAL", Fore.GREEN)
...
async def run_social(language: str):
    from scripts.social import run as run_social
    await run_social(language)

async def run_autoreff(language: str):
    from scripts.autoreff import run as run_autoreff
...
async def run_mint(language: str):
    from scripts.mint import run as run_mint
...
def get_available_scripts(language):
    scripts = {
        'vi': [
            {"name": "1. T\u1ef1 \u0111\u1ed9ng l\u00e0m nhi\u1ec7m v\u1ee5 Social", "value": "social"},
            {"name": "2. Auto Register + Referral + Easy Quests", "value": "autoreff"},
            {"name": "3. K\u1ebft n\u1ed1i Twitter (X OAuth)", "value": "connect"},
            {"name": "4. Claim Equipment", "value": "mint"},
          
            {"name": "X. Tho\u00e1t", "value": "exit"},
        ],
        'en': [
            {"name": "1. Auto complete Social tasks", "value": "social"},
            {"name": "2. Auto Register + Referral + Easy Quests", "value": "autoreff"},
